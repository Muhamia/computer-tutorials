...
```C
typedef unsigned long long size_t;  //unsigned表示其是无符号的，即只能是非负数
```

最大可以表示 18446744073709551615

而int的 2147483647 表示尺寸，在一些情况下可能不够用

同时，`size_t` 相较于直接用 `long long` 还有个优势，就是在32位操作系统下也能正常工作
