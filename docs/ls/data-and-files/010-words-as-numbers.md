...
我们让每一个字符对应一个非负的整数（之后会简称为“整数”），接着就能用一连串的整数来表示一段文字了。我们不妨现在就设计一套表示方式。
...
注意这里用到了一个 $57$ 。在它之前是 `It's me.` ，在它之后是 `My name is LS_Hower.` 。

“换行”看起来好像不太像传统意义上的“字符”，它更像是来控制我们的文字要以怎样的方式显示的。但它的地位确实和其他字符是一样的，它有着自己所对应的一个整数。“换行”是一种 **控制字符** 。
...
上边出现了 $64$ 和 $65$ ，它们就是原句中 `67` 中的 `6` 和 `7` 分别所对应的整数了。

这里要注意的是，比如“7”只不过也是一个普通的字符罢了，和“a”“.”这样的字符并无二致。而我们给它也会分配整数，也就是 67。这个 67 和“7”这个字符本身所具有的语义（比 6 大 1 的数）是没有直接关联的。
...
（什么，你说阿拉伯文是从右往左写的？好吧，这个确实比较棘手，但是这属于深入钻研的时候才需要考虑的问题了。）

## ASCII 与 Unicode

把刚刚看过的东西总结一下。选定一些字符，给每一个都字符分配一个非负整数，这样一来，用一串非负整数就能表示一段文字了。
